...
def save_to_csv(data):
    if data:
        ...
        df.to_csv('dawnData.csv', index=False)
    else:
...
with dag:
    ...
    init_repo_and_dvc = BashOperator(
        task_id='initialize_git_dvc',
        bash_command="""
        cd /mnt/c/Users/Dell/Documents/airflow/dags &&
        if [ ! -d ".git" ]; then
            git init &&
            git remote add origin git@github.com:Anamfatima18/20I1844_MLOPS_ASSIGNMENT-2.git;
        fi &&
        if [ ! -d ".dvc" ]; then
            dvc init &&
